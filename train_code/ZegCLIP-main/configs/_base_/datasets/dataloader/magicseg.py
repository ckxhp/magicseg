...
class MagicSeg(CustomDataset):
    ...
    cat2id = {}
    with open('path/to/magicseg.txt', 'r', encoding='utf-8') as f:
        for line in f:
            if ':' in line:
                CLASSES = [line.split(':')[1].strip() for line in f if ':' in line]
                idx_str, cat = line.rstrip('\n').split(':')
                cat2id[cat.strip()] = idx_str.strip()

    def __init__(self, split, **kwargs):
        super(MagicSeg, self).__init__(
            img_suffix='.jpg', 
            seg_map_suffix='.png', 
            split=split, 
