...
class ZegCLIP(EncoderDecoder):
    ...
    def __init__(self,
        ...
        self.magicseg_to_idx = magicseg_to_idx if magicseg_to_idx is not None else {}
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, neg_img):
        visual_feat = self.extract_feat(img)
        visual_feat_neg = self.extract_feat(neg_img)
        ...
        text_feat_dim = text_feat.size(1)
        
        new_text_feat = torch.zeros(batch_size, 100, text_feat_dim, device=img.device)
        
        all_class_indices = torch.arange(text_feat.size(0), device=img.device)
        
        for i in range(batch_size):
            filename = img_metas[i]['filename']
            filename_without_ext = filename.split('.')[0]
            class_names = filename_without_ext.split('_')
            
            class_indices = []
            for class_name in class_names:
                if class_name in self.magicseg_to_idx:
                    class_indices.append(self.magicseg_to_idx[class_name])
            
            while len(class_indices) < 2:
                class_indices.append(-1)
            
            valid_class_indices = [idx for idx in class_indices if idx != -1]
            
            remaining_indices = [idx for idx in all_class_indices if idx not in valid_class_indices]
            k = 100 - len(valid_class_indices)
            
            if k > 0 and len(remaining_indices) > 0:
                if k <= len(remaining_indices):
                    sampled_indices = torch.tensor(remaining_indices, device=img.device)
                    sampled_indices = sampled_indices[torch.randperm(len(remaining_indices))[:k]]
                else:
                    sampled_indices = torch.tensor(remaining_indices, device=img.device)
                    repeat_times = k // len(remaining_indices)
                    remainder = k % len(remaining_indices)
                    sampled_indices = torch.cat([sampled_indices.repeat(repeat_times), 
                                              sampled_indices[:remainder]])
                
                final_indices = torch.cat([torch.tensor(valid_class_indices, device=img.device), 
                                         sampled_indices])
            else:
                final_indices = torch.tensor(valid_class_indices, device=img.device)

            if len(final_indices) > 100:
                final_indices = final_indices[:100]
            
            new_text_feat[i] = text_feat[final_indices]
        ...
        feat.append(new_text_feat)
        feat.append(visual_feat_neg)
